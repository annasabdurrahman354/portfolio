  ...
  return (
    <div className="space-y-8">
      ...
      {editedContent.experiences.map((exp, idx) => (
        <div key={exp.id} className="p-6 bg-white neo-brutal-border space-y-4 relative group md:pr-32">
          <div className="absolute right-4 top-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
              onClick={() => {
                if (idx > 0) {
                  const newExperiences = [...editedContent.experiences];
                  [newExperiences[idx - 1], newExperiences[idx]] = [newExperiences[idx], newExperiences[idx - 1]];
                  setEditedContent({ ...editedContent, experiences: newExperiences });
                }
              }}
              disabled={idx === 0}
            ...
            <button 
              onClick={() => {
                if (idx < editedContent.experiences.length - 1) {
                  const newExperiences = [...editedContent.experiences];
                  [newExperiences[idx], newExperiences[idx + 1]] = [newExperiences[idx + 1], newExperiences[idx]];
                  setEditedContent({ ...editedContent, experiences: newExperiences });
                }
              }}
              disabled={idx === editedContent.experiences.length - 1}
              className="p-1 hover:bg-gray-100 disabled:opacity-30 bg-white neo-brutal-border"
